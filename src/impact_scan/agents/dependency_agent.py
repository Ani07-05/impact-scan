...
class DependencyAgent(MultiModelAgent):
    ...
    async def _execute_internal(
        ...
        print(f"[{self.name}] Found {len(dep_files)} dependency files: {[f.name for f in dep_files]}")
    ...
    async def _scan_dependencies(
        ...
        for dep_file in dep_files:
            ...
            try:
                ...
                file_findings = [
                    f for f in findings 
                    if str(dep_file) in str(f.file_path) and 
                       self._meets_severity_threshold(f.severity)
                ]
                
                all_findings.extend(file_findings)
                print(f"[{self.name}] Found {len(file_findings)} vulnerabilities in {dep_file.name}")
    ...
    async def _enhance_with_ai_risk_assessment(
        ...
        """
        print(f"[{self.name}] Enhancing {len(findings)} vulnerabilities with AI risk assessment...")
        ...
        for i, finding in enumerate(findings):
            try:
                ...
                print(f"[{self.name}] AI enhanced vulnerability {i+1}/{len(findings)}")
                
            except Exception as e:
                print(f"[{self.name}] AI enhancement failed for vulnerability {i+1}: {e}")
                continue
    
    def _create_risk_assessment_prompt(self, finding: Finding, target_path: Path) -> str:
        """Create AI prompt for vulnerability risk assessment"""
...
"""
    ...
    def _extract_upgrade_recommendation(self, ai_analysis: str) -> str:
        ...
        for line in lines:
            ...
            if any(keyword in line_lower for keyword in ['upgrade to', 'update to', 'version', 'recommended']):
    ...
    def _get_vulnerability_breakdown(self, findings: List[Finding]) -> Dict[str, Any]:
        ...
        for finding in findings:
            ...
            if package_name:
                ...
                breakdown["by_package"][package_name] = breakdown["by_package"].get(package_name, 0) + 1
    ...
    def get_capabilities(self) -> Dict[str, Any]:
        ...
        capabilities.update({
            ...
            "supported_ecosystems": ["python", "javascript", "ruby", "java", "go", "php"],
