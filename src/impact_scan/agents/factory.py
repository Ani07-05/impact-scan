...
class AgentFactory:
    ...
    def discover_agents(self, package_path: str = "impact_scan.agents") -> int:
        """
        ...
        try:
            ...
            for module_info in pkgutil.iter_modules([str(package_dir)]):
                if module_info.name.startswith('_') or module_info.name in ['factory', 'base']:
                    continue
                ...
                try:
                    ...
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (issubclass(obj, Agent) and 
                            obj != Agent and 
                            obj != MultiModelAgent and
                            obj.__module__ == module_name):
                            
                            # Use class name without 'Agent' suffix as type name
                            agent_type = name.lower().replace('agent', '') if name.endswith('Agent') else name.lower()
                            
                            self.register_agent_class(agent_type, obj)
                            discovered += 1
                            print(f"[FACTORY] Discovered agent: {agent_type} ({obj.__name__})")
...
def discover_agents(package_path: str = "impact_scan.agents") -> int:
    """Discover and register agents from a package"""
    return _factory.discover_agents(package_path)

def get_available_agents() -> Dict[str, Dict[str, Any]]:
    """Get all available agent types"""
    return _factory.get_available_agents()
