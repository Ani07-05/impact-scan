...
class AgentOrchestrator:
    ...
    async def execute_comprehensive_scan(
        ...
        plan = await self._create_execution_plan(agents_to_run, strategy, target, context)
    ...
    async def _create_execution_plan(
        ...
        dependencies = [
            ...
            AgentDependency("vuln", depends_on=["recon"], requires_data=["endpoints", "services"], priority=8),
            
            # Exploitation needs vulnerabilities
            AgentDependency("exploit", depends_on=["vuln"], requires_data=["vulnerabilities"], priority=6),
            
            # Compliance can run in parallel with vuln scanning
            AgentDependency("compliance", depends_on=["recon"], priority=7),
            
            # Fix generation should run after vulnerability detection
            AgentDependency("fix", depends_on=["vuln"], requires_data=["vulnerabilities"], priority=4),
        ]
        ...
        for agent_name in sorted_agents:
            if agent_name in self.agents:
                ...
                if result.failed and agent_name in ["recon"]:  # Critical agents
                    print(f"[ORCHESTRATOR] Critical agent {agent_name} failed, stopping execution")
                    break
        ...
        for agent_name in plan.agents:
            if agent_name in self.agents:
                ...
                task = asyncio.create_task(
                    agent.execute(target, agent_context),
    ...
    async def _execute_pipeline(
        ...
        for agent_name in sorted_agents:
            if agent_name in self.agents:
                ...
                print(f"[PIPELINE] Executing {agent_name} with context from {list(results.keys())}")
                
                result = await agent.execute(target, agent_context)
                results[agent_name] = result
                
                # Pipeline can continue even on non-critical failures
                status_msg = "✅ SUCCESS" if result.success else f"❌ FAILED: {result.error_message}"
    ...
    def _build_agent_context(
        ...
        # Add specific data from dependencies
    ...
    def get_orchestration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_agents": [name for name, result in self.results.items() if result.failed],
            "total_findings": sum(len(result.findings) for result in self.results.values()),
            "agent_performance": {
                name: {
