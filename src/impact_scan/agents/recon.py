...
class ReconAgent(MultiModelAgent):
    ...
    async def _execute_internal(
        ...
        print(f"[RECON] Discovered {len(self.discovered_endpoints)} endpoints, "
    ...
    async def _detect_frameworks(self, target_path: Path, result: AgentResult):
        ...
        framework_signatures = {
            ...
            "django": ["manage.py", "settings.py", "from django.conf import", "DJANGO_SETTINGS_MODULE"],
    ...
    async def _analyze_configuration(self, target_path: Path, result: AgentResult):
        ...
        config_files = list(target_path.rglob("*.env")) + \
                ...
                for pattern_name, pattern in sensitive_patterns.items():
                    ...
                    for match in matches:
                        config_findings.append({
                            ...
                            "severity": "high" if pattern_name in ["password", "secret", "api_key"] else "medium"
                        })
    ...
    def _is_network_target(self, target: str) -> bool:
        ...
        try:
            socket.gethostbyname(target.replace("http://", "").replace("https://", "").split("/")[0])
            return True
        except:
            return False
    ...
    async def _enumerate_services(self, target: str, result: AgentResult):
        ...
        for port in common_ports[:5]:  # Limit for demo
            try:
                ...
                if result_code == 0:
                    services[port] = {"status": "open", "service": self._guess_service(port)}
