...
def _select_semgrep_rules(project_context, local_rules_path: Path, scan_root: Path = None) -> List[str]:
    ...
    # Add codebase-specific custom rules if they exist (generated by init-repo)
    if scan_root:
        custom_rules_path = scan_root / ".impact-scan" / "custom-rules.yml"
        if custom_rules_path.exists():
            logger.info(f"Including codebase-specific rules from {custom_rules_path}")
            config_flags.append(f"--config={custom_rules_path}")
...
def run_semgrep_scan(root_path: Path, project_context=None) -> List[schema.Finding]:
    ...
    try:
        ...
        config_flags = _select_semgrep_rules(project_context, local_rules_path, root_path)
