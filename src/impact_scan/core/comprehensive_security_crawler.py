...
from .stealth_crawler import JavaScriptIntelligenceAgent, StealthCrawlingAgent, JavaScriptSecurityData, StealthConfig
...
class ComprehensiveSecurityCrawler:
    ...
    def __init__(self, config: ScanConfig):
        ...
        self.stealth_config = StealthConfig(
            max_concurrency=3,  # Conservative for stealth
        ...
        self.stealth_agent: Optional[StealthCrawlingAgent] = None
    ...
    async def initialize_agents(self):
        ...
        self.stealth_agent = StealthCrawlingAgent(self.config, self.stealth_config)
        
        console.log("[bold green][OSS_INTEL] Comprehensive Security Intelligence Platform initialized[/bold green]")
        console.log(f"[dim]Sources configured: {sum(len(sources) for sources in self.SECURITY_INTELLIGENCE_SOURCES.values())}[/dim]")
        
    async def cleanup_agents(self):
        """Clean up all agent resources."""
        if self.modern_agent:
            await self.modern_agent.cleanup()
        if self.stealth_agent and hasattr(self.stealth_agent, 'cleanup'):
            await self.stealth_agent.cleanup()
            
        console.log("[dim][OSS_CLEANUP] Security Intelligence Platform cleaned up[/dim]")
        
    async def comprehensive_vulnerability_research(self, finding: Finding) -> SecurityIntelligenceReport:
        """
        ...
        console.log(f"[bold cyan][OSS_RESEARCH] Starting comprehensive research for {vuln_id}[/bold cyan]")
        
        # Check cache first
        if vuln_id in self.intelligence_cache:
            cache_report = self.intelligence_cache[vuln_id]
            if self._is_cache_fresh(cache_report):
                console.log(f"[green][CACHE_HIT] Using cached intelligence for {vuln_id}[/green]")
        ...
        with Progress() as progress:
            ...
            synthesis_task = progress.add_task("[green]Intelligence Synthesis...", total=100)
            
            # Phase 1: Static Intelligence Gathering
            console.log(f"[bold blue][PHASE_1] Static intelligence gathering for {vuln_id}[/bold blue]")
            report.static_intelligence = await self._gather_static_intelligence(finding, progress, static_task)
            
            # Phase 2: JavaScript-Heavy Site Intelligence
            console.log(f"[bold blue][PHASE_2] JavaScript intelligence gathering for {vuln_id}[/bold blue]")
            report.javascript_intelligence = await self._gather_javascript_intelligence(finding, progress, js_task)
            
            # Phase 3: Threat Landscape Analysis
            console.log(f"[bold blue][PHASE_3] Threat landscape analysis for {vuln_id}[/bold blue]")
            report.threat_landscape = await self._analyze_threat_landscape(finding, progress, threat_task)
            
            # Phase 4: Intelligence Synthesis
            console.log(f"[bold blue][PHASE_4] Intelligence synthesis for {vuln_id}[/bold blue]")
    ...
    async def _gather_static_intelligence(self, finding: Finding, progress: Progress, task_id: TaskID) -> SecurityIntelligence:
        """Gather intelligence from static sources."""
        progress.update(task_id, advance=10, description="[cyan]Initializing static intelligence...")
        
        intelligence = await self.modern_agent.research_vulnerability(finding)
        
        progress.update(task_id, advance=90, description="[cyan]Static intelligence complete")
        return intelligence
        
    async def _gather_javascript_intelligence(self, finding: Finding, progress: Progress, task_id: TaskID) -> List[JavaScriptSecurityData]:
        """Gather intelligence from JavaScript-heavy sources."""
        progress.update(task_id, advance=10, description="[blue]Identifying JS-heavy sources...")
        
        # Identify JavaScript-heavy security sources that require special handling
        js_heavy_sources = [
            f"https://github.com/search?q={finding.vuln_id}&type=repositories",
            f"https://nvd.nist.gov/vuln/detail/{finding.vuln_id}",
            f"https://security.snyk.io/vuln/{finding.vuln_id}",
            f"https://www.exploit-db.com/search?q={finding.vuln_id}",
        ]
        
        progress.update(task_id, advance=30, description="[blue]Crawling JS-heavy sources...")
        
        js_intelligence = await self.stealth_agent.stealth_crawl_security_sources(
            js_heavy_sources, finding.vuln_id
        )
        
        progress.update(task_id, advance=60, description="[blue]JavaScript intelligence complete")
        return js_intelligence
        
    async def _analyze_threat_landscape(self, finding: Finding, progress: Progress, task_id: TaskID) -> Dict[str, Any]:
        """Analyze threat landscape and real-world exploitation."""
        progress.update(task_id, advance=20, description="[red]Analyzing threat landscape...")
        ...
        progress.update(task_id, advance=40, description="[red]Analyzing vendor responses...")
        ...
        progress.update(task_id, advance=30, description="[red]Threat analysis complete")
    ...
    async def _synthesize_intelligence(self, report: SecurityIntelligenceReport, progress: Progress, task_id: TaskID):
        """Synthesize all gathered intelligence into actionable insights."""
        progress.update(task_id, advance=20, description="[green]Synthesizing intelligence...")
        
        # Generate PoC examples
        report.poc_examples = self._extract_poc_examples(report)
        
        progress.update(task_id, advance=20, description="[green]Generating insights...")
    ...
    def _extract_poc_examples(self, report: SecurityIntelligenceReport) -> List[Dict[str, Any]]:
    ...
    def _generate_actionable_insights(self, report: SecurityIntelligenceReport) -> List[str]:
        """Generate actionable insights for security teams."""
        insights = []
        
        # Severity-based insights
        if report.finding.severity == Severity.CRITICAL:
            insights.append("🚨 CRITICAL: Immediate action required - this vulnerability poses severe risk")
            
        # Exploit availability insights
        exploit_count = len(report.static_intelligence.exploits)
        if exploit_count > 0:
            insights.append(f"⚠️ EXPLOITATION: {exploit_count} known exploits available - prioritize patching")
            
        # Patch availability insights
        patch_count = len(report.static_intelligence.patches)
        if patch_count > 0:
            insights.append(f"✅ PATCHES: {patch_count} vendor patches available - apply immediately")
        else:
            insights.append("❌ NO_PATCHES: No official patches found - consider workarounds")
            
        # PoC availability insights
        poc_count = len(report.poc_examples)
        if poc_count > 0:
            insights.append(f"🛠️ POC_AVAILABLE: {poc_count} proof-of-concept examples found")
            
        # Confidence insights
        if report.confidence_score > 0.8:
            insights.append("✅ HIGH_CONFIDENCE: Intelligence gathered from reliable sources")
        elif report.confidence_score < 0.5:
            insights.append("⚠️ LOW_CONFIDENCE: Limited intelligence available - manual research recommended")
            
        return insights
        
    def _calculate_risk_assessment(self, report: SecurityIntelligenceReport) -> Dict[str, float]:
        ...
        final_risk = min(base_severity + exploit_factor + poc_factor + patch_factor + confidence_factor, 1.0)
    ...
    def _generate_vulnerability_timeline(self, report: SecurityIntelligenceReport) -> List[Dict[str, Any]]:
    ...
    def _calculate_confidence_score(self, report: SecurityIntelligenceReport) -> float:
        ...
        if report.javascript_intelligence:
            js_confidence = sum(0.7 if data.security_advisories else 0.3 
                               for data in report.javascript_intelligence) / len(report.javascript_intelligence)
            scores.append(js_confidence)
            
        # Threat landscape confidence
        if report.threat_landscape:
            threat_confidence = 0.8 if report.threat_landscape.get('exploit_availability') else 0.6
            scores.append(threat_confidence)
            
        return sum(scores) / len(scores) if scores else 0.0
        
    def _calculate_completeness_score(self, report: SecurityIntelligenceReport) -> float:
    ...
    def _is_cache_fresh(self, report: SecurityIntelligenceReport, max_age_hours: int = 24) -> bool:
    ...
    def _count_exploits_in_content(self, content: str) -> int:
        ...
        exploit_keywords = ['exploit', 'poc', 'proof of concept', 'metasploit', 'nuclei']
        count = 0
    ...
    async def generate_comprehensive_report(self, findings: List[Finding]) -> Dict[str, Any]:
        """
        ...
        """
        console.log(f"[bold cyan][OSS_PLATFORM] Generating comprehensive report for {len(findings)} findings[/bold cyan]")
        
        reports = []
        
        # Process findings with concurrency control
        semaphore = asyncio.Semaphore(self.stealth_config.max_concurrency)
        ...
        aggregate_stats = {
            ...
            'total_sources_analyzed': sum(len(r.static_intelligence.sources) for r in reports),
            'total_exploits_found': sum(len(r.static_intelligence.exploits) for r in reports),
            ...
            'average_completeness': sum(r.completeness_score for r in reports) / len(reports),
            'high_risk_count': sum(1 for r in reports if r.risk_assessment.get('final_risk_score', 0) > 0.7),
    ...
    def _generate_intelligence_summary(self, reports: List[SecurityIntelligenceReport]) -> Dict[str, Any]:
        """Generate high-level intelligence summary."""
        return {
            'critical_vulnerabilities': [r.vulnerability_id for r in reports if r.finding.severity == Severity.CRITICAL],
            'exploitable_vulnerabilities': [r.vulnerability_id for r in reports if len(r.static_intelligence.exploits) > 0],
            'patchable_vulnerabilities': [r.vulnerability_id for r in reports if len(r.static_intelligence.patches) > 0],
            'research_coverage': {
                'fully_researched': len([r for r in reports if r.completeness_score > 0.8]),
                'partially_researched': len([r for r in reports if 0.5 <= r.completeness_score <= 0.8]),
                'limited_research': len([r for r in reports if r.completeness_score < 0.5])
            }
        }


# Export main class
__all__ = ['ComprehensiveSecurityCrawler', 'SecurityIntelligenceReport', 'ThreatIntelligence']