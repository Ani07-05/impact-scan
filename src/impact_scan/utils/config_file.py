    ...
    if 'ai_provider' in config or 'ai' in config:
        ...
        if isinstance(provider, str):
            try:
                ...
                raise ValueError(f"Invalid AI provider '{provider}'. Available: {available}")
    
    # Boolean flags
    for key in ['enable_ai_fixes', 'ai_fixes', 'enable_web_search', 'web_search']:
        if key in config:
            validated[key.replace('_', '_').replace('ai_fixes', 'enable_ai_fixes').replace('web_search', 'enable_web_search')] = bool(config[key])
